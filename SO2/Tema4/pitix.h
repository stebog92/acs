...
static inline long get_data_size(struct super_block *sb)
{
	struct pitix_super_block *psb=(struct pitix_super_block*)sb->s_fs_info;
	return sizeof(__u16)*psb->inode_data_blocks;
}

static inline void get_offsets(int ino, int *block_id,
							 int *block_offset, struct super_block *s)
{
	struct pitix_super_block *psb=(struct pitix_super_block*)s->s_fs_info;
	*block_id = psb->izone_block + ino * inode_size(s) / s->s_blocksize;
	*block_offset = ino * inode_size(s) % s->s_blocksize;
}
...
/* Inode operations */
extern struct inode* pitix_new_inode(struct inode *dir, umode_t mode);
extern int pitix_write_inode(struct inode * inode, struct
		writeback_control *wbc);
extern void pitix_delete_inode(struct inode *inode);
extern void pitix_clear_inode(struct inode *inode);
