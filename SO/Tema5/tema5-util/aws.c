...
static enum connection_state send_message(struct connection *conn)
{
    ...
    sendfile(conn->sockfd, fd, NULL, stat_info.st_size); 

    w_epoll_remove_fd(epollfd, fd);
...
static void handle_client_request(struct connection *conn)
{
    ...
    strcat(buffer, request_path);
    
    fd = open(buffer, O_RDONLY);
    if (fd == -1) {
        dlog(LOG_ERR, "file not opened");
    } else {
        dlog(LOG_ERR, "file opened");
        strcpy(conn->send_buffer, "HTTP/1.1 200 OK\r\n\r\n");
    }
    fstat(fd, &stat_info); 
    w_epoll_add_fd_out(epollfd, fd);
