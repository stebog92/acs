...
void transitive_closure(GraphAdjMat& graph)
{
  ...
  for (unsigned int k = 0; k < graph.get_n(); ++k) {
    for (unsigned int i = 0; i < graph.get_n(); ++i) {
      for (unsigned int j = 0; j < graph.get_n(); ++j) {
        /* Trebuie sa existe neaparat drum:
         * a) de la I la K
         * b) de la K la J
         * pentru a putea adauga eventual, in inchiderea tranzitiva, muchie
         * directa de la I la J. */
        if (graph.get_edge(i,k) != GraphAdjMat::NONE &&
            graph.get_edge(k,j) != GraphAdjMat::NONE) {
          if (graph.get_edge(i,k) + graph.get_edge(k,j) < graph.get_edge(i,j) ||
              graph.get_edge(i,j) == GraphAdjMat::NONE) {
...
void print_routing_table(GraphAdjMat& graph)
{
  ...
  for (unsigned int n = 0; n < graph.get_n(); ++n) {
    std::cout << "The table of node: " << n << std::endl;
    for (unsigned int dest = 0; dest < graph.get_n(); ++dest) {
      if (graph.get_edge(n, dest) != GraphAdjMat::NONE) {
        std::cout << "Dest: " << std::setw(5) << dest << " | "
            << "Next hop: " << std::setw(5)
            << (graph.get_detour(n, dest) == -1 ?
                dest :
                graph.get_detour(n, dest))
            << " | "
            << "Total cost: " << std::setw(5) << graph.get_edge(n, dest)
            << std::endl;
      }
    }
    std::cout << std::endl;
...
void dijkstra(GraphAdjMat& graph, int source, int dest)
{
  ...
  for (unsigned int i = 0; i < graph.get_n(); ++i) {
    ...
    for (int j = 0; j < graph.get_n(); ++j) {
      if (dmin[j] != GraphAdjMat::NONE && parent[j] != GraphAdjMat::NONE) {
        if (nodmin == GraphAdjMat::NONE || dmin[nodmin] > dmin[j]) {
          nodmin = j;
        }
      }
    }

    /* Daca nu s-a gast nici un nod pana la care sa existe un drum, atunci
     * break. */
    if (nodmin == GraphAdjMat::NONE) {
      return;
    }

    /* Altfel adaugam ruta (source, nodmin) cu:
     * a) ponderea muchiei egala cu dmin[nodmin]
     * b) detour-ul pe (source, nodmin) prin parent[nodmin] */
    graph.set_edge(source, nodmin, dmin[nodmin]);

    /* Stabilim detour doar daca nodul parinte nu este deja sursa (caz in care
     * detour-ul este redundant). */
    if (parent[nodmin] != source) {
      graph.set_detour(source, nodmin, parent[nodmin]);
    }

    /* Scoatem nodul din calcul anulandu-i parintele si apoi relaxam celelalte
     * muchii care pleaca din el. */
    parent[nodmin] = GraphAdjMat::NONE;
    for (int j = 0; j < graph.get_n(); ++j) {
      if (graph.get_edge(nodmin, j) != GraphAdjMat::NONE &&
          (graph.get_edge(nodmin, j) + dmin[nodmin] < dmin[j] ||
           dmin[j] == GraphAdjMat::NONE)) {
        dmin[j] = dmin[nodmin] + graph.get_edge(nodmin, j);
        parent[j] = nodmin;
      }
    }
  }
  ...
  for (int i = 0; i < graph.get_n(); ++i) {
    for (int j = 0; j < graph.get_n(); ++j) {
      if (i != j && used[i][j] == false &&
          graph.get_edge(i, j) != GraphAdjMat::NONE) {
        /* Calculam un ciclu minim care contine muchia (i, j). */
        GraphAdjMat::Path cycle = minimum_cycle(graph, i, j);

        /* Retinem sa nu mai calculam cicluri pentru muchiile din acest ciclu. */
        for (int k = 0; k < cycle.size(); ++k) {
          used[cycle[k].first.first][cycle[k].first.second] = true;
          used[cycle[k].first.second][cycle[k].first.first] = true;
        }

        /* Adaugam ciclul la valoarea de return. */
        if (cycle.size()) {
          returnValue.push_back(cycle);
        }
      }
    }
  }
