...
public class Lab6p2 {
    ...
    static void dfs(Node node) {
        /*
         * TODO: Puteti folosi aceasta functie pentru a implementa o parcurgere
         * in adancime (in cadrul careia sa completati si timpii de finish).
         */
        time++;
        node.color = Color.GRAY;
        for (Node nextNode : node.neigh) {
            if (nextNode.color == Color.WHITE) {
                dfs(nextNode);
            }
        }
    ...
    static void planYears(Graph graph, Vector<Vector<String>> yearlyPlanning) {
         ...
         *
         * (a) Daca o materie A depinde de o materie B, atunci anul in care se
         * face materia A trebuie sa fie _strict_ mai mare decat anul in care se
         * face materia B.
         *
         ...
         *
         * In varaibila yearly planning veti pune in ordine, vectori de
         * stringuri care contin materiile ce vor fi parcurse in anii
         * consecutivi de studiu: 1, 2, 3, etc.
         */
        for (Node n : graph)
            n.finishTime = 0;
        ...
        for (int i = 0; i < graph.size(); i++) {
            if (graph.get(i).inDegree == 0) {
                S.addLast(graph.get(i));
            }
        }
        while (!S.isEmpty()) {
            Node u = S.removeLast();
            while (u.finishTime + 1 > yearlyPlanning.size())
                yearlyPlanning.add(new Vector<String>());
            for (int i = 0; i < yearlyPlanning.size(); i++) {
                if (yearlyPlanning.get(i).size() < 5 && u.finishTime <= i) {
                    yearlyPlanning.get(i).add(u.subjectName);
                    u.finishTime = i;
                    break;
                }
            }
            Vector<Node> neighbours = new Vector<Node> (u.neigh);
            for (Node vec : neighbours) {
                u.neigh.remove(vec);
                vec.inDegree--;
                if (vec.finishTime < u.finishTime + 1) {
                        vec.finishTime = u.finishTime + 1;
                    }
                if (vec.inDegree == 0) {
                    S.addLast(vec);
                }
    ...
    public static void main(String[] args) throws FileNotFoundException {
        ...
        for (int i = 0; i < edgeCount; ++i) {
             ...
             */
            graph.get(subjectToInt.get(subjectA)).neigh.add(graph.get(subjectToInt.get(subjectB)));
